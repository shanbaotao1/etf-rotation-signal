...
最后更新: 2026-08-02
本次改动: 修复云端调仓「按固定 TOTAL_CAPITAL 计算股数」导致的隐性加杠杆 bug。原云端分支用恒定 5 万本金算股数，
          净值跌破本金后换仓即透支买入（实测 7/28 现金 -2252 元 ≈ 4.7% 杠杆，7/31 已达 -1810 元），
          等于亏损时自动加杠杆、盈利时仓位闲置（反向凯利），系统性放大回撤。
          ...
          注：本地版 run_strategy 早已使用 total_cash 滚动，不受影响；历史净值曲线口径不变（仅影响未来股数）。
前次改动: 2026-07-30 修复单持仓「同 ETF 连续持有时每日按现价重算股数」的幽灵缩股 bug。
"""
...
def _available_capital(prev_positions, quotes):
    ...
    首次运行（无 stats.json）回退 TOTAL_CAPITAL。
    ...
    if cash is None:
        # 旧版 stats 无 cash 字段：用净值近似（等价于满仓全部变现）
