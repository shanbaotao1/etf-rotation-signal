...
"""
def build_html(v1_stats, wf_stats, bj_str):
...
<title>双策略对比 · 轮动V1 vs 五福</title>
<style>{CSS}</style></head>
<body><div class="wrap">
<h1>📈 双策略实盘对比 · 轮动V1 vs 五福</h1>
<div class="sub">更新于 {bj_str} ｜ 两策略均自 2026-07-23 起、初始资金 ¥50,000、成本统一（佣金万0.5无最低 + 单边0.1%滑点）｜ 基准：{BENCHMARK_NAME}</div>
...
<div class="card">
  ...
  <div class="note">* 超额收益 = 策略累计收益 − {BENCHMARK_NAME}累计收益（正=跑赢基准）。曲线：红=轮动V1，紫=五福，蓝虚线=沪深300。</div>
</div>
...
"""
    ...
    "const V1 = DATA.v1.daily, WF = DATA.wufu.daily;\n"
    "let curRange = 'all';\n"
    "function parseDate(s){ return new Date(s+'T00:00:00'); }\n"
    "function fmtPct(x){ return (x*100).toFixed(2)+'%'; }\n"
    "function unionDates(){ const s=new Set(); V1.forEach(d=>s.add(d.date)); WF.forEach(d=>s.add(d.date)); return Array.from(s).sort(); }\n"
    "function rangeDates(){\n"
    ...
    "  const vVals=arrOf(vmap), wVals=arrOf(wmap), iVals=arrOf(imap);\n"
    "  let mn=0,mx=0;\n"
    "  for(const arr of [vVals,wVals,iVals]) for(const v of arr) if(v!==null){ if(v<mn)mn=v; if(v>mx)mx=v; }\n"
    ...
    "             '<rect x=\"'+(padL+80)+'\" y=\"5\" width=\"11\" height=\"11\" fill=\"#7b1fa2\"/>'+\n"
    "             '<text x=\"'+(padL+95)+'\" y=\"14\" font-size=\"11\" fill=\"#444\">五福</text>'+\n"
    "             '<rect x=\"'+(padL+150)+'\" y=\"5\" width=\"11\" height=\"11\" fill=\"#1565c0\"/>'+\n"
    "             '<text x=\"'+(padL+165)+'\" y=\"14\" font-size=\"11\" fill=\"#444\">__BN__</text>';\n"
    "  el.innerHTML='<svg viewBox=\"0 0 '+W+' '+H+'\" width=\"100%\" preserveAspectRatio=\"xMidYMid meet\">'+legend+grid+\n"
    "    poly(iVals,'#1565c0',true)+poly(vVals,'#e53935',false)+poly(wVals,'#7b1fa2',false)+xlab+'</svg>';\n"
    "}\n"
    ...
    "function update(){\n"
    "  const rd=rangeDates();\n"
    "  const s1=computeStats(sliceOf(V1,rd)), s2=computeStats(sliceOf(WF,rd));\n"
    ...
    "  const idxTxt = (s1&&s1.idxTotalRet!==null)?fmtPct(s1.idxTotalRet):(s2&&s2.idxTotalRet!==null?fmtPct(s2.idxTotalRet):'—');\n"
    "  const ie=document.getElementById('rIdx'); ie.textContent=idxTxt; ie.className=(s1&&s1.idxTotalRet!==null&&s1.idxTotalRet>=0)?'pos':'neg';\n"
    "  document.getElementById('rFrom').textContent = rd.length?rd[0]:'-';\n"
    "  document.getElementById('rTo').textContent = rd.length?rd[rd.length-1]:'-';\n"
    "  drawChart(rd); renderTable('tbV1',V1,rd); renderTable('tbWf',WF,rd);\n"
    "}\n"
...
def main():
    ...
    html = build_html(v1, wf, bj)
    open(OUT_HTML, 'w', encoding='utf-8').write(html)
    _log(f"已写 index.html（{bj}），V1日={len(v1['daily']) if v1 else 0} 五福日={len(wf['daily']) if wf else 0}")
